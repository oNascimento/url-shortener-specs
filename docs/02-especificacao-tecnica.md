...
[openapi.json](openapi.json) é o contrato OpenAPI 3.1 complementar. A API usa JSON camelCase, UTC em RFC 3339 e Problem Details em `application/problem+json` com code e traceId. Não expor stack trace, SQL ou segredos. Respostas autenticadas e de autenticação incluem Cache-Control: no-store.
