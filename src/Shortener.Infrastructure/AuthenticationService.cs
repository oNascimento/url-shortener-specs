...
public sealed class AuthService(AppDbContext db, UserManager<ApplicationUser> users, TimeProvider clock)
{
    ...
    { var email = input.Email.Trim(); var user = new ApplicationUser { Id = Guid.NewGuid(), UserName = email, Email = email, NormalizedEmail = email.ToUpperInvariant(), NormalizedUserName = email.ToUpperInvariant(), CreatedAt = clock.GetUtcNow() }; var r = await users.CreateAsync(user, input.Password); return r.Succeeded ? (user, await CreateActionToken(user, "verify_email", TimeSpan.FromHours(24), ct)) : null; }
